...
| aider "forgets" the file you gave it | Ollama's 2k default context | Raise it on the server. macOS: `launchctl setenv OLLAMA_CONTEXT_LENGTH 8192` and restart the app — `ollama serve` will fail with *address already in use* |
| `connection refused` from aider | Nothing serving on 11434 | macOS: open the Ollama app. Windows/Linux: `ollama serve`, and leave that terminal open |
