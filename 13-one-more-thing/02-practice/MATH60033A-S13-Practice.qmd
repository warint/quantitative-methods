...
format:
  revealjs:
    ...
    margin: 0.06
    smaller: true
    footer: "MATH60033A · Session 13 practice · Thierry Warin, PhD · HEC Montréal"
---
...
mkdir -p groups/A2026/group-XX/session-13      # macOS and Linux
```

```powershell
git checkout -b group-XX
mkdir groups/A2026/group-XX/session-13         # Windows — no -p
```
...
Take the result the paper rests on, and put it to the test on the database the paper itself uses. There is nothing to unzip — the loader has it.

```python
import qmib

data = qmib.load("kof")     # the session's own data
```

::: {.muted}
Paper: Gygli, Haelg, Potrafke & Sturm (2019), *The KOF Globalisation Index — revisited*, Review of International Organizations 14(3) · data: `qmib.load("kof")`
:::
...
**[warin.ca/qmib-labs](https://warin.ca/qmib-labs/qmib-lab-07.html)**
:::
