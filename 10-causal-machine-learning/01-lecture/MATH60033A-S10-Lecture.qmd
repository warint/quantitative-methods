...
education  <- round(rnorm(n, mean = 14, sd = 2))
experience <- pmax(0, age - 18 + rnorm(n, 0, 3))

# Treatment assignment with confounding

# Older and more educated individuals have a slightly higher probability of treatment.
ps <- plogis(-5 + 0.05*age + 0.15*education)
treat <- rbinom(n, 1, ps)

# Outcome model with a true treatment effect of +3
outcome <- 20 +
0.3*age +
0.8*education +
0.5*experience +
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
library(MatchIt)
m.out <- matchit(treat ~ age + education + experience, 
                 data = df, 
                 method = "nearest")
matched <- match.data(m.out)
t.test(outcome ~ treat, data = matched)
```
:::
...
ps_model <- glm(treat ~ age + education + experience, 
                data = df, 
                family = binomial())
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
# Propensity score distributions: before matching
plot(density(fitted(ps_model)[df$treat == 1]),
     main = "Propensity Score Distributions (Before Matching)",
     xlab = "Propensity Score")
lines(density(fitted(ps_model)[df$treat == 0]), col = 2)
...
plot(density(matched_df$distance[matched_df$treat == 1]),
     main = "Propensity Score Distributions (After Matching)",
     xlab = "Propensity Score")
lines(density(matched_df$distance[matched_df$treat == 0]), col = 2)
legend("topright", legend = c("Treated", "Control"),
       col = c(1, 2), lty = 1)
```
:::
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
# Boxplots of age before and after matching
boxplot(df$age ~ df$treat,
        main = "Before Matching: Age",
        xlab = "Treatment status", ylab = "Age")
```
:::
::::
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
boxplot(matched_df$age ~ matched_df$treat,
        main = "After Matching: Age",
        xlab = "Treatment status", ylab = "Age")
```
:::
::::
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
# 1. Get the matched data
matched_df <- match.data(m.out)

# matched_df already contains all original columns from df
# including 'outcome', plus 'treat', 'distance', and 'weights'.

# 2. Estimate ATT on the matched sample
att_model <- lm(outcome ~ treat,
                data   = matched_df,
                weights = weights)
...
``` {.sourceCode .numberSource .r .number-lines .code-with-copy}
mean(df$outcome[df$treat == 1]) - mean(df$outcome[df$treat == 0])
```
:::
