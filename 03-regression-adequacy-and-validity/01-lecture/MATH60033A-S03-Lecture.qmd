...
3. Models: Assessing the adequacy
4. Models: assessing the validity
5. Model Robustness/Parsimony and Selection
...
## Models: Assessing the adequacy (cont.) {#imported-s03-008 .scrollable}
...
## what makes it the more useful of the two to quote to a non-specialist. (cont.) {#imported-s03-011 .scrollable}
...
## Models: assessing the validity (cont.) {#imported-s03-014 .scrollable}
...
## Models: assessing the validity (cont.) {#imported-s03-015 .scrollable}
...
## Normality of Residuals (cont.) {#imported-s03-017 .scrollable}
...
## Residuals vs Fitted: is there structure the straight line missed? (cont.) {#imported-s03-019 .scrollable}
...
# One spike per observation: how much every coefficient would move if that
# single row were deleted. The dashed line is the conventional 4/n screen —
# a prompt to go and look at the row, not a rule for dropping it.
plt.figure(figsize=(7, 4))
plt.stem(cooks_d)
