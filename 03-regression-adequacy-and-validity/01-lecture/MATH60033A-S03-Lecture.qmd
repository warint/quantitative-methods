...
Cook's distance answers one question directly: **if I deleted this row, how far would all the
fitted values move?** It does that by combining the two ingredients from the previous slide — how
badly the point is predicted, and how much leverage it has — into a single number per observation.

$$D_i = \\frac{e_i^2}{p\\,s^2} \\cdot \\frac{h_i}{(1-h_i)^2}$$

The first factor is the squared residual: the outlier part. The second is built from $h_i$, the
leverage: it is small when the point sits in the middle of the X range and blows up as $h_i$
approaches one. Multiply them and you have exactly the "both at once" case that does the damage.
...
::: {.muted}
`get_influence()` computes leverage, studentised residuals and Cook's distance together, because
they are all read off the same hat matrix.
:::

::: {.warn}
$4/n$ is a **screen, not a test**. It has no distributional justification; it is a threshold that
flags roughly the most extreme few per cent in a well-behaved sample. Crossing it is a reason to
go and look at the row, never a reason to delete it.
:::
