...
- Report the comparison as a publication table with `qmib.regtable`
...
Both appear in the formula above, and they are not the same quantity.

| | measures the spread of | with more data |
|---|---|---|
| **standard deviation** $\sigma$ | the **observations**, around their mean | does **not** shrink — you estimate a fixed $\sigma$ more precisely |
| **standard error** $\mathrm{SE}(\hat\mu) = \sigma/\sqrt{n}$ | an **estimate**, around the truth, across repeated samples | shrinks as $1/\sqrt{n}$ |

::: {.step}
They carry the same units, which is why they get swapped. But $\sigma$ is a fact
about the population your sample came from, and $\mathrm{SE}$ is a fact about
*your estimate of it*: the standard error **is** the standard deviation of
$\hat\mu$ over the samples you might have drawn but did not.
:::

::: {.warn}
So: an error bar that stays the same width as $n$ grows is a **standard
deviation**; one that narrows is a **standard error**. Collecting more data
makes you more certain *where the mean is*; it does not make the observations
less spread out.
:::
...
::::::: columns
::: {.column style="width:50%;"}
The residuals should be **nearly** normal — and nearly is the operative word. Least squares does
not need normal errors to give you unbiased coefficients; it needs them for the **t-tests and
confidence intervals** to mean what they claim. In a large sample even that requirement softens,
because the central limit theorem does the work for you.

So the question is not "are these residuals normal" — they never are — but **how badly does the
departure distort the inference**. Heavy tails on both sides usually signal a handful of unusual
observations rather than a broken model; a single long tail on one side usually means the
dependent variable wanted a log.
...
Three words that get used interchangeably in conversation and mean quite different things here.
Keeping them apart is most of the work.

| | | |
|---|---|---|
| **Outlier** | unusual **Y** given X | the model predicts it badly |
| **Leverage point** | unusual **X** | it sits far from the other predictor values |
| **Influential point** | removing it **moves the coefficients** | the answer depends on it |

A point can be any one of these without being the others. An outlier sitting in the middle of the
X range is badly predicted but has little power to tilt the line: there are neighbours on both
sides holding it in place. A leverage point that happens to fall exactly where the line already
goes is unusual in X and perfectly consistent with everything else — it does no damage at all,
and in fact it *stabilises* the slope.

::: {.warn}
The damage is done by points that are **both**. Far out in X, so the line pivots around them, and
badly predicted, so the pivot is large. That combination is what influence means, and it is why
neither residual size nor leverage on its own is the diagnostic you want.
:::

::: {.muted}
Which is the argument for a measure that multiplies the two together. That measure is Cook's
distance, next.
:::

## Cook's Distance {#imported-s03-024 .scrollable}

One question, asked of every row in turn: **delete this observation, refit, and how far do all
the fitted values move?** Cook's distance is the answer, and it is a single number per row.

$$D_i \;=\; \underbrace{\frac{e_i^2}{p\,s^2}}_{\text{is it badly predicted?}} \;\times\;
            \underbrace{\frac{h_i}{(1-h_i)^2}}_{\text{does it have leverage?}}$$

Those are the two ingredients from the previous slide, multiplied rather than added — which is
the point. A big residual in the middle of the X range gives a large first factor and a tiny
second one, so the product stays small. It takes **both** to move the line, and only the product
knows that.
...
::: {.warn}
$4/n$ is a **screen, not a test**. It has no distributional justification — it simply flags the
most extreme few per cent of a well-behaved sample. Crossing it is a reason to go and look at the
row. It is never, on its own, a reason to delete it.
:::

## One spike per observation {.scrollable}

One spike per row, its height the distance the fitted values would travel if that row were
deleted. Read it for **shape**, not for threshold-crossing: a forest of similar spikes means no
single country-year is carrying the result, while one tower above the rest means your finding is
really a statement about one observation.
...
::: {.step}
When a spike stands out, the next move is always the same: **find out which row it is** and go and
look at it.

```python
d.iloc[cooks_d.argmax()]      # the observation, not just its index
```
:::

::: {.warn}
Then ask what it is. A data-entry error you fix. A small economy that genuinely behaves
differently is not an error — it is a finding, and dropping it to tidy the plot is how a result
gets manufactured. If you do exclude a row, the exclusion and the reason for it belong in the
write-up, alongside the fit with the row left in.
:::

## Parsimony · comparing candidate models {.center background-color="#E3120B" style="color:#F7F4ED"}

## Information criteria --- AIC and BIC {#imported-s03-025 .scrollable}

Adding a variable can never make the fit worse, so "which model fits best" always answers
"the biggest one". Information criteria fix that by **charging rent** for each parameter:

$$\text{AIC} = -2\log(L) + 2k \qquad\qquad \text{BIC} = -2\log(L) + k\ln(n)$$

The first term is the same in both — it is the fit, and it falls as the model grows. The second
term is the rent, and it is the only thing that differs:

| | Rent per parameter | Which means |
|---|---|---|
| **AIC** | $2$ | a flat charge, whatever the sample size |
| **BIC** | $\ln(n)$ | the charge rises with $n$ — at $n = 410$ it is $6.02$, three times AIC's |

::: {.warn}
So BIC always prefers a model at least as small as AIC's, and the gap widens with the sample.
Neither is "correct": AIC is aiming at the model that predicts best, BIC at the model that is
true. Those are different targets, and they are allowed to disagree.
:::

## Watching them disagree {.scrollable}

Four nested models on the same 410 rows — because two fits estimated on different samples are not
a comparison.
...
cols = ["gdp_pc_eur", "productivity_idx", "gfcf_meur", "employment_ths", "falling_behind"]
dic  = core.dropna(subset=cols)

specs = {
    "productivity":     "productivity_idx",
    "+ investment":     "productivity_idx + gfcf_meur",
    "+ employment":     "productivity_idx + gfcf_meur + employment_ths",
    "+ falling_behind": "productivity_idx + gfcf_meur + employment_ths + falling_behind",
}
fits = {k: smf.ols(f"gdp_pc_eur ~ {v}", data=dic).fit() for k, v in specs.items()}

ic = pd.DataFrame({"k":   [int(m.df_model) + 1 for m in fits.values()],
                   "AIC": [m.aic for m in fits.values()],
                   "BIC": [m.bic for m in fits.values()]}, index=list(fits))
ic["dAIC"] = ic.AIC - ic.AIC.min()      # distance from the best, which is what you read
ic["dBIC"] = ic.BIC - ic.BIC.min()

print(f"n = {len(dic)},  AIC's rent = 2,  BIC's rent = ln(n) = {np.log(len(dic)):.2f}\n")
print(ic.round(2).to_string())
print(f"\nAIC prefers: {ic.AIC.idxmin()}\nBIC prefers: {ic.BIC.idxmin()}")
```

## Reading that table {.scrollable}

The first two steps are unanimous: investment and employment are worth their rent several times
over, and both criteria drop by more than a hundred points.

The last step is where they part. Adding `falling_behind` improves AIC by 0.74 and *worsens* BIC
by 3.28 — and the difference between those two movements is not a coincidence:

$$3.28 - (-0.74) \;=\; 4.02 \;=\; \ln(410) - 2$$

That is the extra rent BIC charges for one more parameter. **The entire disagreement is that one
number.** The data has not changed; the price of complexity has.

::: {.check}
Read **differences, not levels.** An AIC of 8442 means nothing on its own — it depends on the
units of $y$ and the size of the sample. A $\Delta$AIC of 0.74 between two models means they are
indistinguishable; a $\Delta$ of 145 means one is out of the running. A rough convention: under 2
is a tie, over 10 is decisive.
:::

::: {.warn}
And the standing condition: **same rows, same $y$.** Two AICs computed on different samples, or
one on $y$ and one on $\log y$, are not comparable at all — and `dropna()` silently changes the
sample every time you change the column list.
:::

## The table you would actually publish {.scrollable}

`qmib.regtable` is R's **stargazer**, here: one column per model, stars on the
coefficients, standard errors beneath, fit statistics at the foot.
...
# Both models on the SAME rows. dropna gives a different sample for each set of
# columns, and two fits estimated on different samples are not a comparison —
# their AIC, their R-squared and their coefficients are not on one scale.
m1 = smf.ols("gdp_pc_eur ~ productivity_idx", data=d2).fit()
m2 = smf.ols("gdp_pc_eur ~ productivity_idx + gfcf_meur", data=d2).fit()
...
::: {.muted}
`.as_latex()` for a paper, `.as_html()` for a slide. Pass your own `stats=`
dict to put different rows at the foot.
:::

## Choosing a model is not a diagnostic
...
::: {.step}
It also cannot be judged on a case like this one. With five candidates and 367 rows forward
selection simply keeps all five, and nothing is learned. The procedure only *looks* decisive when
there are many candidates and few rows --- which is the case it is actually used in, and the case
where it is least trustworthy.
:::

::: {.check}
So it opens **session 05** instead, on 29 candidates and 37 rows, where it does real work: watch
its choices fail to survive a resample and you have the argument for shrinking coefficients
continuously rather than including or excluding them. That argument is ridge and lasso.
:::
...
you have just sat through: **[warin.ca/qmib-labs](https://warin.ca/qmib-labs/qmib-lab-03.html)**
:::
