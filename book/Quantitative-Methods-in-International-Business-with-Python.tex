...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-02-nightingale.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-02_files/figure-pdf/fig-table-vs-chart-output-1.png}}
...
\caption{}\label{T_befe3}\tabularnewline
\toprule\noalign{}
...
One more distinction belongs here, because the same square root is used
for two different jobs and the two are confused constantly --- including
in published tables.

\begin{qmibdefinition}

{\textbf{Standard deviation}} --- \(s = \sqrt{s^2}\), the spread of the
\textbf{observations} around their mean, in the units of the variable.

\end{qmibdefinition}

\begin{qmibdefinition}

{\textbf{Standard error}} --- \(\mathrm{SE}(\bar x) = s/\sqrt{n}\), the
spread of an \textbf{estimate} around the truth, across the samples you
might have drawn but did not. It estimates \(\sigma/\sqrt{n}\).

\end{qmibdefinition}

They share units and differ by a factor most people do not carry in
their head, which is why they get swapped. What separates them is what
each is the spread \emph{of}: \(s\) describes the data, \(\mathrm{SE}\)
describes a number computed from the data.

The consequence is a test you can apply by eye. As \(n\) grows, \(s\)
settles on a fixed \(\sigma\) --- observations do not become less
variable because you collected more of them --- while \(\mathrm{SE}\)
shrinks as \(1/\sqrt{n}\) without limit. So an error bar that stays the
same width as the sample grows is a standard deviation, and one that
narrows is a standard error. More data tells you more precisely where
the mean is; it tells you nothing new about how spread out the
observations are.
...
\pandocbounded{\includegraphics[keepaspectratio]{session-02_files/figure-pdf/fig-distribution-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-02_files/figure-pdf/fig-missing-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-02_files/figure-pdf/fig-triangle-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-03-gauss.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-03-legendre.jpg}}
...
\(\mathrm{E}[\varepsilon \mid X] = 0\), and if the errors have constant
variance and are mutually uncorrelated, then \(\hat\beta\) is the Best
Linear Unbiased Estimator: no other estimator that is both linear in
\(y\) and unbiased has smaller variance. Note what is \emph{not} on that
list. Normality is not required for Gauss--Markov. It is required for
something else --- the exactness of the \(t\) and \(F\) distributions in
small samples --- and confusing the two is one of the most common errors
in applied work.
...
\pandocbounded{\includegraphics[keepaspectratio]{session-03_files/figure-pdf/fig-galton-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-03_files/figure-pdf/fig-anscombe-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-03_files/figure-pdf/fig-diagnostics-output-1.png}}
...
\(\sum_i h_{ii} = \mathrm{tr}(H) = p\), so leverage is a fixed budget of
\(p\) shared among \(n\) observations. The average is exactly \(p/n\),
and each \(h_{ii}\) lies in \([0,1]\). The conventional flag is
...
\pandocbounded{\includegraphics[keepaspectratio]{session-03_files/figure-pdf/fig-influence-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-03_files/figure-pdf/fig-qq-output-1.png}}
...
wrong. What breaks is \(\mathrm{Var}(\hat\beta)\): the usual
...
absurd --- the fitted values estimate \(\mathrm{E}[y \mid x]\), which
for a binary variable is exactly \(\Pr(y = 1 \mid x)\), and the
...
\(\mathrm{Var}(y_i) = p_i(1 - p_i)\) by construction. The variance is a
\emph{function of the mean}, largest at \(p = 0.5\) and vanishing at
either end. Heteroscedasticity is not a defect of the data here; it is a
property of binary outcomes, and no amount of clean data removes it. The
coefficients remain unbiased --- chapter 3's table applies --- but the
classical standard errors are wrong, always, and robust standard errors
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-04-verhulst.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-04_files/figure-pdf/fig-logistic-output-1.png}}
...
\(\partial p / \partial x_j = \beta_j\thinspace p(1-p)\), the change in
...
\pandocbounded{\includegraphics[keepaspectratio]{session-05_files/figure-pdf/fig-constraint-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-05_files/figure-pdf/fig-path-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-06_files/figure-pdf/fig-heterogeneity-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-07-pearson.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-07_files/figure-pdf/fig-pearson-line-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-07_files/figure-pdf/fig-scree-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-07_files/figure-pdf/fig-biplot-output-2.png}}
...
\(\mathrm{E}[\varepsilon] = 0\) and
\(\mathrm{Var}(\varepsilon) = \sigma^2\). Let \(\hat f\) be an estimate
built from a random training sample. The expected squared error at a
point \(x_0\), averaged over training samples and over the noise in a
new observation, splits into three:
...
\pandocbounded{\includegraphics[keepaspectratio]{session-08_files/figure-pdf/fig-bias-variance-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-08_files/figure-pdf/fig-choose-k-output-2.png}}
...
\caption{}\label{T_3d97f}\tabularnewline
\toprule\noalign{}
...
\pandocbounded{\includegraphics[keepaspectratio]{session-09_files/figure-pdf/fig-path-diagram-output-1.png}}
...
\section{What the estimator assumes}\label{what-the-estimator-assumes}

Nothing above said how \(\theta\) is chosen. The default, in
\texttt{semopy} as in \texttt{lavaan}, is \textbf{maximum likelihood},
and the likelihood it maximises is the multivariate normal one. That
assumption is not decoration: the \(\chi^2\) statistic and every
standard error in the output are derived from it.

When the data are not multivariate normal --- and survey items with
floor or ceiling effects rarely are --- the failure runs in one
direction, which is what makes it dangerous. The \(\chi^2\) statistic is
inflated, so a model that fits is rejected. The standard errors are too
small, so paths look more significant than they are. Non-normality does
not make you cautious. It makes you confident.

The assumption is checked at two levels, and the second is not implied
by the first. Univariately, a Shapiro--Wilk test on each indicator.
Multivariately --- which is the assumption actually being made ---
Mardia's measures of skewness and kurtosis, built from the Mahalanobis
cross-products \(d_{ij} = (x_i - \bar x)^\top S^{-1} (x_j - \bar x)\):
...
\(k(k+1)(k+2)/6\) degrees of freedom, and \(b_2\) has mean \(k(k+2)\)
--- every indicator can pass Shapiro--Wilk while the joint distribution
fails both.

\phantomsection\label{check-mardia}
\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ scipy }\ImportTok{import}\NormalTok{ stats}

\NormalTok{indicators }\OperatorTok{=}\NormalTok{ [}\StringTok{"price"}\NormalTok{, }\StringTok{"resale\_value"}\NormalTok{, }\StringTok{"maintenance"}\NormalTok{, }\StringTok{"fuel\_efficiency"}\NormalTok{,}
              \StringTok{"safety"}\NormalTok{, }\StringTok{"space\_comfort"}\NormalTok{, }\StringTok{"technology"}\NormalTok{, }\StringTok{"after\_sales\_service"}\NormalTok{]}
\NormalTok{X }\OperatorTok{=}\NormalTok{ survey[indicators].to\_numpy(}\BuiltInTok{float}\NormalTok{)}
\NormalTok{n, k }\OperatorTok{=}\NormalTok{ X.shape}

\CommentTok{\# The cross{-}product matrix. Its diagonal holds each row\textquotesingle{}s squared Mahalanobis}
\CommentTok{\# distance from the centroid; the off{-}diagonal entries are what make b1 a}
\CommentTok{\# measure of *joint* skewness rather than eight separate ones.}
\NormalTok{Xc }\OperatorTok{=}\NormalTok{ X }\OperatorTok{{-}}\NormalTok{ X.mean(}\DecValTok{0}\NormalTok{)}
\NormalTok{D }\OperatorTok{=}\NormalTok{ Xc }\OperatorTok{@}\NormalTok{ np.linalg.inv(np.cov(X, rowvar}\OperatorTok{=}\VariableTok{False}\NormalTok{, bias}\OperatorTok{=}\VariableTok{True}\NormalTok{)) }\OperatorTok{@}\NormalTok{ Xc.T}

\NormalTok{b1 }\OperatorTok{=}\NormalTok{ (D }\OperatorTok{**} \DecValTok{3}\NormalTok{).}\BuiltInTok{sum}\NormalTok{() }\OperatorTok{/}\NormalTok{ n }\OperatorTok{**} \DecValTok{2}          \CommentTok{\# sum over every pair i, j}
\NormalTok{b2 }\OperatorTok{=}\NormalTok{ (np.diag(D) }\OperatorTok{**} \DecValTok{2}\NormalTok{).}\BuiltInTok{sum}\NormalTok{() }\OperatorTok{/}\NormalTok{ n      }\CommentTok{\# diagonal only — the squared distances}

\NormalTok{chi2, df }\OperatorTok{=}\NormalTok{ n }\OperatorTok{*}\NormalTok{ b1 }\OperatorTok{/} \DecValTok{6}\NormalTok{, k }\OperatorTok{*}\NormalTok{ (k }\OperatorTok{+} \DecValTok{1}\NormalTok{) }\OperatorTok{*}\NormalTok{ (k }\OperatorTok{+} \DecValTok{2}\NormalTok{) }\OperatorTok{//} \DecValTok{6}
\NormalTok{z }\OperatorTok{=}\NormalTok{ (b2 }\OperatorTok{{-}}\NormalTok{ k }\OperatorTok{*}\NormalTok{ (k }\OperatorTok{+} \DecValTok{2}\NormalTok{)) }\OperatorTok{/}\NormalTok{ np.sqrt(}\DecValTok{8} \OperatorTok{*}\NormalTok{ k }\OperatorTok{*}\NormalTok{ (k }\OperatorTok{+} \DecValTok{2}\NormalTok{) }\OperatorTok{/}\NormalTok{ n)}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"n = }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\SpecialStringTok{, k = }\SpecialCharTok{\{}\NormalTok{k}\SpecialCharTok{\}}\SpecialStringTok{ indicators}\CharTok{\textbackslash{}n}\SpecialStringTok{"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"skewness  b1 = }\SpecialCharTok{\{}\NormalTok{b1}\SpecialCharTok{:6.3f\}}\SpecialStringTok{   chi2 = }\SpecialCharTok{\{}\NormalTok{chi2}\SpecialCharTok{:6.1f\}}\SpecialStringTok{  df = }\SpecialCharTok{\{}\NormalTok{df}\SpecialCharTok{\}}\SpecialStringTok{"}
      \SpecialStringTok{f"   p = }\SpecialCharTok{\{}\NormalTok{stats}\SpecialCharTok{.}\NormalTok{chi2}\SpecialCharTok{.}\NormalTok{sf(chi2, df)}\SpecialCharTok{:.4f\}}\SpecialStringTok{"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"kurtosis  b2 = }\SpecialCharTok{\{}\NormalTok{b2}\SpecialCharTok{:6.2f\}}\SpecialStringTok{   expected }\SpecialCharTok{\{}\NormalTok{k }\OperatorTok{*}\NormalTok{ (k }\OperatorTok{+} \DecValTok{2}\NormalTok{)}\SpecialCharTok{\}}\SpecialStringTok{"}
      \SpecialStringTok{f"          z = }\SpecialCharTok{\{}\NormalTok{z}\SpecialCharTok{:5.2f\}}\SpecialStringTok{   p = }\SpecialCharTok{\{}\DecValTok{2} \OperatorTok{*}\NormalTok{ stats}\SpecialCharTok{.}\NormalTok{norm}\SpecialCharTok{.}\NormalTok{sf(}\BuiltInTok{abs}\NormalTok{(z))}\SpecialCharTok{:.4f\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
skewness  b1 = 11.911   chi2 =  178.7  df = 120   p = 0.0004
kurtosis  b2 =  85.18   expected 80          z =  1.94   p = 0.0522
\end{verbatim}

Which is the uncomfortable part. The measurement model fitted earlier in
this chapter was estimated by maximum likelihood on data whose joint
skewness rejects normality at \(p < 0.001\), with kurtosis borderline.
Everything the next section says about its fit is therefore computed
from a statistic the data do not quite license --- and this is the
normal situation with survey data, not a contrived one.

The answer is not to abandon the model but to change the estimator.
\textbf{Robust maximum likelihood} --- \texttt{MLR}, resting on the
Satorra--Bentler scaled \(\chi^2\) --- keeps the ML point estimates
unchanged and rescales the test statistic and the standard errors by the
non-normality actually observed. The paths do not move; the claims made
about them get weaker, which is the correction being asked for. For
ordinal indicators, \textbf{WLSMV} goes further and treats a five-point
item as a coarsened continuous variable rather than pretending its
categories are numbers.

So report the estimator alongside the fit. \texttt{ML} and \texttt{MLR}
on one model return the same coefficients and different \(p\)-values,
and on data like these only one of the two is defensible.
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-10-hume.jpg}}
...
{\textbf{ATE}} --- average treatment effect, \(\mathrm{E}[Y(1) - Y(0)]\)
over the whole population. What would happen if everyone were treated
versus no one.
...
\(\mathrm{E}[Y(1) - Y(0) \mid T = 1]\). What the treatment did for those
who actually received it. Usually the policy-relevant quantity, and
usually not equal to the ATE.
...
\(\lbrace Y(0), Y(1)\rbrace \perp T \mid X\). Every determinant of both
treatment and outcome is in \(X\). \textbf{This assumption is not
testable}, because it concerns the missing potential outcome.
...
\pandocbounded{\includegraphics[keepaspectratio]{session-10_files/figure-pdf/fig-balance-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/session-11-snow.jpg}}
...
\(t\), with \(g \in \lbrace\text{treated}, \text{control}\rbrace\) and
\(t \in \lbrace\text{pre}, \text{post}\rbrace\). The estimator is
...
\section{The estimator has a name}\label{the-estimator-has-a-name}

The loop in that cell is not an improvisation. Estimating each cohort
against a clean comparison group and aggregating with weights you chose
is precisely the procedure of Callaway and Sant'Anna
(\citeproc{ref-callaway2021}{2021}), and there is a Python
implementation of it, \texttt{diff-diff}. It is a young package and the
reference implementation is still R's \texttt{did}, so the first thing
to do with it is what this section does: run it beside a calculation you
already understand, and find out where the two disagree.

That is worth doing here for three reasons. It states out loud which
cells it refuses to estimate. It attaches standard errors to the cohort
estimates, which the hand-rolled loop had no way to produce. And it does
disagree --- in a way that turns out to be the hand-rolled version's
fault.

\phantomsection\label{check-cs}
\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ diff\_diff }\ImportTok{as}\NormalTok{ dd}

\CommentTok{\# The estimator wants one column holding the adoption year, and 0 — not a}
\CommentTok{\# sentinel it would read as a year — for the never{-}treated.}
\NormalTok{panel[}\StringTok{"g"}\NormalTok{] }\OperatorTok{=}\NormalTok{ panel[}\StringTok{"cohort"}\NormalTok{].replace(}\DecValTok{9999}\NormalTok{, }\DecValTok{0}\NormalTok{)}

\NormalTok{cs }\OperatorTok{=}\NormalTok{ dd.CallawaySantAnna(cluster}\OperatorTok{=}\StringTok{"geo"}\NormalTok{).fit(}
\NormalTok{    panel, outcome}\OperatorTok{=}\NormalTok{Y, unit}\OperatorTok{=}\StringTok{"geo"}\NormalTok{, time}\OperatorTok{=}\StringTok{"time"}\NormalTok{, first\_treat}\OperatorTok{=}\StringTok{"g"}\NormalTok{)}
\NormalTok{gt }\OperatorTok{=}\NormalTok{ cs.to\_dataframe()}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\StringTok{\textquotesingle{}cohort\textquotesingle{}}\SpecialCharTok{:\textgreater{}7\}\{}\StringTok{\textquotesingle{}year\textquotesingle{}}\SpecialCharTok{:\textgreater{}6\}\{}\StringTok{\textquotesingle{}ATT(g,t)\textquotesingle{}}\SpecialCharTok{:\textgreater{}10\}\{}\StringTok{\textquotesingle{}SE\textquotesingle{}}\SpecialCharTok{:\textgreater{}7\}}\SpecialStringTok{"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ \_, r }\KeywordTok{in}\NormalTok{ gt.iterrows():}
\NormalTok{    g, t }\OperatorTok{=} \BuiltInTok{int}\NormalTok{(r.group), }\BuiltInTok{int}\NormalTok{(r.time)}
    \ControlFlowTok{if}\NormalTok{ pd.isna(r.effect):}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{g}\SpecialCharTok{:\textgreater{}7\}\{}\NormalTok{t}\SpecialCharTok{:\textgreater{}6\}\{}\StringTok{\textquotesingle{}—\textquotesingle{}}\SpecialCharTok{:\textgreater{}10\}\{}\StringTok{\textquotesingle{}—\textquotesingle{}}\SpecialCharTok{:\textgreater{}7\}}\SpecialStringTok{   dropped: }\SpecialCharTok{\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{skip\_reason}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        tag }\OperatorTok{=} \StringTok{"  ← pre{-}treatment, not an effect"} \ControlFlowTok{if}\NormalTok{ t }\OperatorTok{\textless{}}\NormalTok{ g }\ControlFlowTok{else} \StringTok{""}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{g}\SpecialCharTok{:\textgreater{}7\}\{}\NormalTok{t}\SpecialCharTok{:\textgreater{}6\}\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{effect}\SpecialCharTok{:\textgreater{}10.3f\}\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{se}\SpecialCharTok{:\textgreater{}7.3f\}\{}\NormalTok{tag}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialCharTok{\{}\StringTok{\textquotesingle{}overall ATT\textquotesingle{}}\SpecialCharTok{:\textgreater{}18\}\{}\NormalTok{cs}\SpecialCharTok{.}\NormalTok{overall\_att}\SpecialCharTok{:\textgreater{}10.3f\}\{}\NormalTok{cs}\SpecialCharTok{.}\NormalTok{overall\_se}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{"}
      \SpecialStringTok{f"   bias }\SpecialCharTok{\{}\NormalTok{cs}\SpecialCharTok{.}\NormalTok{overall\_att }\OperatorTok{{-}}\NormalTok{ TRUE\_EFFECT}\SpecialCharTok{:+.3f\}}\SpecialStringTok{, }\SpecialCharTok{\{}\NormalTok{cs}\SpecialCharTok{.}\NormalTok{n\_clusters}\SpecialCharTok{\}}\SpecialStringTok{ clusters"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ \_, r }\KeywordTok{in}\NormalTok{ cs.aggregate(}\StringTok{"group"}\NormalTok{).to\_dataframe().iterrows():}
\NormalTok{    n }\OperatorTok{=}\NormalTok{ panel.loc[panel[}\StringTok{"cohort"}\NormalTok{] }\OperatorTok{==} \BuiltInTok{int}\NormalTok{(r.label), }\StringTok{"geo"}\NormalTok{].nunique()}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\StringTok{\textquotesingle{}cohort \textquotesingle{}} \OperatorTok{+} \BuiltInTok{str}\NormalTok{(r.label)}\SpecialCharTok{:\textgreater{}18\}\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{att}\SpecialCharTok{:\textgreater{}10.3f\}\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{se}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{"}
          \SpecialStringTok{f"   }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\SpecialStringTok{ countr}\SpecialCharTok{\{}\StringTok{\textquotesingle{}y\textquotesingle{}} \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{==} \DecValTok{1} \ControlFlowTok{else} \StringTok{\textquotesingle{}ies\textquotesingle{}}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}

\CommentTok{\# The hand{-}rolled estimates and these ones differ. Recomputing each 2x2 over}
\CommentTok{\# only the countries observed in *both* of its two years isolates why.}
\BuiltInTok{print}\NormalTok{(}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{the hand{-}rolled numbers, and what was wrong with them:"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\StringTok{\textquotesingle{}g,t\textquotesingle{}}\SpecialCharTok{:\textgreater{}9\}\{}\StringTok{\textquotesingle{}as written\textquotesingle{}}\SpecialCharTok{:\textgreater{}12\}\{}\StringTok{\textquotesingle{}same units\textquotesingle{}}\SpecialCharTok{:\textgreater{}12\}\{}\StringTok{\textquotesingle{}ATT(g,t)\textquotesingle{}}\SpecialCharTok{:\textgreater{}10\}}\SpecialStringTok{"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ g, t }\KeywordTok{in}\NormalTok{ ((}\DecValTok{2022}\NormalTok{, }\DecValTok{2022}\NormalTok{), (}\DecValTok{2022}\NormalTok{, }\DecValTok{2023}\NormalTok{), (}\DecValTok{2022}\NormalTok{, }\DecValTok{2024}\NormalTok{), (}\DecValTok{2023}\NormalTok{, }\DecValTok{2023}\NormalTok{)):}
\NormalTok{    sub }\OperatorTok{=}\NormalTok{ panel[panel[}\StringTok{"cohort"}\NormalTok{].isin([g, }\DecValTok{9999}\NormalTok{]) }\OperatorTok{\&}\NormalTok{ panel[}\StringTok{"time"}\NormalTok{].isin([g }\OperatorTok{{-}} \DecValTok{1}\NormalTok{, t])]}
\NormalTok{    seen }\OperatorTok{=}\NormalTok{ sub.groupby(}\StringTok{"geo"}\NormalTok{)[}\StringTok{"time"}\NormalTok{].nunique()}
\NormalTok{    bal }\OperatorTok{=}\NormalTok{ sub[sub[}\StringTok{"geo"}\NormalTok{].isin(seen[seen }\OperatorTok{==} \DecValTok{2}\NormalTok{].index)]}
\NormalTok{    tr }\OperatorTok{=}\NormalTok{ bal[bal[}\StringTok{"cohort"}\NormalTok{] }\OperatorTok{==}\NormalTok{ g].groupby(}\StringTok{"time"}\NormalTok{)[Y].mean()}
\NormalTok{    co }\OperatorTok{=}\NormalTok{ bal[bal[}\StringTok{"cohort"}\NormalTok{] }\OperatorTok{==} \DecValTok{9999}\NormalTok{].groupby(}\StringTok{"time"}\NormalTok{)[Y].mean()}
\NormalTok{    fixed }\OperatorTok{=}\NormalTok{ (tr[t] }\OperatorTok{{-}}\NormalTok{ tr[g }\OperatorTok{{-}} \DecValTok{1}\NormalTok{]) }\OperatorTok{{-}}\NormalTok{ (co[t] }\OperatorTok{{-}}\NormalTok{ co[g }\OperatorTok{{-}} \DecValTok{1}\NormalTok{])}
\NormalTok{    att }\OperatorTok{=}\NormalTok{ gt.loc[(gt.group }\OperatorTok{==}\NormalTok{ g) }\OperatorTok{\&}\NormalTok{ (gt.time }\OperatorTok{==}\NormalTok{ t), }\StringTok{"effect"}\NormalTok{].iloc[}\DecValTok{0}\NormalTok{]}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{g}\SpecialCharTok{\}}\SpecialStringTok{,}\SpecialCharTok{\{}\NormalTok{t}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}}\SpecialCharTok{:\textgreater{}9\}\{}\NormalTok{twoby(g, g }\OperatorTok{{-}} \DecValTok{1}\NormalTok{, t)}\SpecialCharTok{:\textgreater{}12.3f\}\{}\NormalTok{fixed}\SpecialCharTok{:\textgreater{}12.3f\}\{}\NormalTok{att}\SpecialCharTok{:\textgreater{}10.3f\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
 cohort  year  ATT(g,t)     SE
   2021  2021         —      —   dropped: missing_period
   2021  2022         —      —   dropped: missing_period
   2021  2023         —      —   dropped: missing_period
   2021  2024         —      —   dropped: missing_period
   2022  2022     1.739  1.135
   2022  2023     2.751  0.843
   2022  2024     2.138  0.977
   2023  2022    -1.126  0.440  ← pre-treatment, not an effect
   2023  2023     1.842  0.553
   2023  2024         —      —   dropped: zero_treated_control

       overall ATT     2.196  0.782   bias -0.204, 30 clusters
       cohort 2022     2.209  0.809   9 countries
       cohort 2023     1.842  0.553   1 country

the hand-rolled numbers, and what was wrong with them:
      g,t  as written  same units  ATT(g,t)
2022,2022       1.701       1.739     1.739
2022,2023       2.702       2.751     2.751
2022,2024       2.292       2.138     2.138
2023,2023       1.666       1.842     1.842
\end{verbatim}

The first table is this chapter's own reasoning, produced by the
estimator rather than argued for in advance. The 2021 cohort is dropped
as \texttt{missing\_period} --- no pre-treatment observation exists,
which is the conclusion reached two sections ago by reading the adoption
structure. ATT(2023, 2024) is dropped as
\texttt{zero\_treated\_control}, the same absent country the hand-rolled
version returned \texttt{NaN} for. Neither refusal is buried inside a
coefficient.

The standard errors are new. They come from the influence function of
each group-time effect, clustered by country, and they carry through to
the aggregate: an overall ATT of +2.20 with a standard error of 0.78,
against a truth of 2.40. The interval covers it, which is the honest
summary of what thirty countries over four years can establish about an
effect of this size.

\begin{qmibwarn}

\textbf{Read the cohort lines again.} The nine-country cohort reports a
standard error of 0.81. The one-country cohort reports 0.55. One
country's trajectory is not measured more precisely than nine countries'
--- the smaller number is what comes out when there is no
between-country variation left to estimate, and the software has no way
to know that a cohort of one should not be reported at all. The
estimator computes whatever you ask it for. Deciding what is worth
asking for is still yours.

\end{qmibwarn}

The last table is the uncomfortable one. The hand-rolled estimates and
Callaway--Sant'Anna's differ --- +1.70 against +1.74, +2.29 against
+2.14 --- and the middle column says why. \texttt{twoby} averaged
whichever countries happened to be present in each year, and this panel
is not balanced: Malta and Estonia have no 2022 observation, Luxembourg
no 2021, Spain no 2023, Croatia neither. The ``before'' mean and the
``after'' mean were therefore computed over different sets of countries,
and part of what looked like an effect was the change in who was being
averaged. Hold the units fixed and the estimator is reproduced to three
decimals.

That failure is on this chapter's own list --- it is the
\textbf{composition change} warning further down, committed in code
written to demonstrate the method, on a dataset small enough to audit by
hand. Which is the argument for using the package rather than the
argument against it: not that it is cleverer, but that the choices the
hand-rolled version made silently are ones it is forced to make out
loud.
...
\pandocbounded{\includegraphics[keepaspectratio]{session-11_files/figure-pdf/fig-event-study-output-2.png}}
...
\section{How large a violation would it
take?}\label{how-large-a-violation-would-it-take}

``The assumption cannot be checked'' is where this dataset leaves you.
It is not where the literature leaves you. Rambachan and Roth
(\citeproc{ref-rambachan2023}{2023}) changes the question: rather than
testing parallel trends, ask how large a violation of it would have to
be before the finding stops holding, and report \emph{that} number. The
output is a family of intervals indexed by how much post-treatment
departure from parallel trends you are prepared to allow --- measured,
in the relative-magnitudes version, as a multiple \(M\) of the largest
departure actually visible before treatment.

\phantomsection\label{check-honest}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{es }\OperatorTok{=}\NormalTok{ cs.aggregate(}\StringTok{"event\_study"}\NormalTok{)}

\BuiltInTok{print}\NormalTok{(}\StringTok{"the whole of the pre{-}treatment evidence in this dataset:"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ \_, r }\KeywordTok{in}\NormalTok{ es.to\_dataframe().query(}\StringTok{"event\_time \textless{} 0"}\NormalTok{).iterrows():}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"   event time }\SpecialCharTok{\{}\BuiltInTok{int}\NormalTok{(r.event\_time)}\SpecialCharTok{:\textgreater{}3\}}\SpecialStringTok{: }\SpecialCharTok{\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{att}\SpecialCharTok{:+.3f\}}\SpecialStringTok{  "}
          \SpecialStringTok{f"(SE }\SpecialCharTok{\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{se}\SpecialCharTok{:.3f\}}\SpecialStringTok{, p }\SpecialCharTok{\{}\NormalTok{r}\SpecialCharTok{.}\NormalTok{p\_value}\SpecialCharTok{:.3f\}}\SpecialStringTok{), from }\SpecialCharTok{\{}\BuiltInTok{int}\NormalTok{(r.n)}\SpecialCharTok{\}}\SpecialStringTok{ cohort"}\NormalTok{)}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialCharTok{\{}\StringTok{\textquotesingle{}M\textquotesingle{}}\SpecialCharTok{:\textgreater{}5\}\{}\StringTok{\textquotesingle{}identified set\textquotesingle{}}\SpecialCharTok{:\textgreater{}22\}\{}\StringTok{\textquotesingle{}95}\SpecialCharTok{\% r}\StringTok{obust CI\textquotesingle{}}\SpecialCharTok{:\textgreater{}22\}}\SpecialStringTok{   rules out zero"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ M }\KeywordTok{in}\NormalTok{ (}\FloatTok{0.0}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{1.0}\NormalTok{, }\FloatTok{2.0}\NormalTok{):}
\NormalTok{    h }\OperatorTok{=}\NormalTok{ dd.HonestDiD(method}\OperatorTok{=}\StringTok{"relative\_magnitude"}\NormalTok{, M}\OperatorTok{=}\NormalTok{M).fit(es)}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{M}\SpecialCharTok{:\textgreater{}5.2f\}}\SpecialStringTok{   [}\SpecialCharTok{\{}\NormalTok{h}\SpecialCharTok{.}\NormalTok{lb}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{,}\SpecialCharTok{\{}\NormalTok{h}\SpecialCharTok{.}\NormalTok{ub}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{]     "}
          \SpecialStringTok{f"[}\SpecialCharTok{\{}\NormalTok{h}\SpecialCharTok{.}\NormalTok{ci\_lb}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{,}\SpecialCharTok{\{}\NormalTok{h}\SpecialCharTok{.}\NormalTok{ci\_ub}\SpecialCharTok{:\textgreater{}7.3f\}}\SpecialStringTok{]   "}
          \SpecialStringTok{f"}\SpecialCharTok{\{}\StringTok{\textquotesingle{}yes\textquotesingle{}} \ControlFlowTok{if}\NormalTok{ h}\SpecialCharTok{.}\NormalTok{is\_significant }\ControlFlowTok{else} \StringTok{\textquotesingle{}no\textquotesingle{}}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
the whole of the pre-treatment evidence in this dataset:
   event time  -1: -1.126  (SE 0.440, p 0.016), from 1 cohort

    M        identified set         95% robust CI   rules out zero
 0.00   [  2.212,  2.212]     [  0.613,  3.812]   yes
 0.25   [  1.649,  2.776]     [  0.050,  4.375]   yes
 0.50   [  1.086,  3.339]     [ -0.513,  4.938]   no
 1.00   [ -0.040,  4.465]     [ -1.640,  6.064]   no
 2.00   [ -2.293,  6.718]     [ -3.892,  8.317]   no
\end{verbatim}

Two things to read off that, and the second matters more.

The pre-treatment evidence is a single number, from a single cohort, and
that cohort is a single country. Germany adopted in 2023, so it is the
only unit whose pre-period is something other than its own reference
year, and its 2021→2022 change ran 1.13 points below the never-treated
countries'. Everything in the table below rests on that one comparison.
The 2022 cohort, nine countries strong, contributes nothing here at all
--- its only pre-period \emph{is} the reference.

The sensitivity table then says: the result survives at \(M = 0.25\) and
not at \(M = 0.5\). In words, if post-treatment departures from parallel
trends could be as much as half the size of the one pre-treatment
departure visible in the data, the interval already contains zero. That
is a low breakdown point, and it ought to be --- it is what an estimate
resting on one country's single pre-period is worth.

\begin{qmibwarn}

\textbf{This is not a test, and nothing here has been tested.} The
honest sentence is ``the finding requires post-treatment violations of
parallel trends no larger than about a quarter of the single
pre-treatment deviation we can observe, and we have one country from
which to judge that.'' A sensitivity analysis converts an untestable
assumption into a statement about how much of it the result needs. That
is real progress over asserting the assumption, and it is not the same
...
Where the pre-trend evidence is thin --- and outside a handful of long
panels it usually is --- Rambachan and Roth
(\citeproc{ref-rambachan2023}{2023}) replaces the ritual pre-trend test
with a sensitivity analysis. Report the range of estimates consistent
with violations of parallel trends up to a stated size, and the reader
can see how much of the assumption the finding actually needs rather
...
\item
  ...
  decomposition is the finding. \texttt{diff-diff}'s
  \texttt{CallawaySantAnna} is the Python route; the \texttt{did}
...
\item
  \textbf{How much violation the result survives.} A breakdown value ---
  the size of departure from parallel trends at which the finding stops
  holding --- is worth more than a pre-trend \(p\)-value, and unlike the
  \(p\)-value it can be reported even when there is nothing to test.
\end{enumerate}
...
many times larger. A three-billion-parameter model on a laptop is not
...
The course installs the 3-billion variant rather than the 7, and the
reason is worth stating because it is the same reasoning in miniature.
Most student laptops have no dedicated graphics card, so the model runs
on the processor. On a processor the 7B is not a better model: it
produces a comparable answer to the same question at roughly a quarter
of the speed, and a tool slow enough to interrupt your thinking is a
tool you stop using. The binding constraint is not quality, it is
whether you will actually reach for it.
...
\section{Two ways of using it,
deliberately}\label{two-ways-of-using-it-deliberately}

The same model is reached through two different tools, and the split is
not redundancy.

\textbf{Continue} puts the model in the editor. You select code and ask
about it, and the answer arrives beside the file. This is the mode that
matters most in a course where you will read far more code than you
write --- the session decks, the authors' replication packages, your
group's scripts. The friction of copying code into a browser is small,
and it is enough to stop most people asking.

\textbf{aider} puts the model in the terminal, inside your repository,
where it can change files and commit what it changed. That is a
different kind of act, and the course treats it as one. You see every
command it proposes before it runs, and whatever it writes is committed
under your name --- which is the point, since the commit history is one
of the records used to check that all three members of a group did the
work. Delegating the typing does not delegate the answerability.

The honest limitation, stated once: on a machine without a graphics
card, a small model is a capable reader and an unreliable editor. It
will explain a function correctly and then fail to produce a well-formed
multi-file edit, because emitting a precise diff is a harder task than
describing code. That is a fact about small models rather than about
you, and the practical response is to use aider for understanding and do
the editing yourself --- which is, in any case, the position the course
would prefer you were in.
...
\subsection{What your machine needs}\label{what-your-machine-needs}

No graphics card is required. Everything runs on the processor, more
slowly.
...
CPU & anything from the last five years & Apple Silicon, or Intel i7
10th gen and later \\
...
\textbf{CPU only, \texttt{qwen2.5-coder:3b}} & 8--20 & a few seconds \\
CPU only, \texttt{qwen2.5-coder:7b} & 3--8 & half a minute or more \\
\end{longtable}

\textbf{Use the 3b unless \texttt{ollama\ ps} reports
\texttt{100\%\ GPU}.} On a processor the 7b is not a better model --- it
is the same answer, four times slower.

\subsubsection{Per tool}\label{per-tool}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Ollama} & 8 GB for a 3b, 16 GB for a 7b & \textasciitilde2 GB
per model & A model needs its file size plus about a fifth, in memory \\
\textbf{Continue} & the model's, plus nothing much & \textasciitilde50
MB & Does \textbf{not} manage the context window --- set it yourself \\
\textbf{aider} & as Ollama & its own environment via pipx & Sets the
context window per request itself \\
\end{longtable}

\textbf{aider on a CPU-only machine:} fine for reading and asking,
unreliable for editing. Small models often fail to produce its edit
format and report \emph{``the LLM did not conform to the edit format''}
instead of making a change. If you want it to edit anyway,
\texttt{-\/-edit-format\ whole} asks it to rewrite files rather than
...
\item[$\square$]
  \textbf{Python 3.12.10 specifically} --- not ``the latest'' --- from
  \url{https://www.python.org/downloads/release/python-31210/}; on
  Windows tick \emph{``Add python.exe to PATH''}. Later 3.12 releases
  are security-only and ship as source code with no installer, and
  \texttt{aider-chat} requires Python \texttt{\textless{}3.13}, so 3.13
  and 3.14 cannot run this course's toolchain
\item[$\square$]
  \texttt{python3.12\ -\/-version} prints 3.12.x --- on Windows
  \texttt{py\ -3.12\ -\/-version}, and \texttt{py\ -\/-list} shows every
  Python you have. Build the environment with the version named
  (\texttt{py\ -3.12\ -m\ venv\ .venv},
  \texttt{python3.12\ -m\ venv\ .venv}), because a bare \texttt{py} or
  \texttt{python3} picks the newest one installed
\item[$\square$]
...
\item[$\square$]
  A model pulled: \textbf{\texttt{qwen2.5-coder:3b}} --- the default,
  and the one that works without a dedicated graphics card. Try
  \texttt{:7b} only after \texttt{ollama\ ps} shows \textbf{100\% GPU}
  on the 3b
\item[$\square$]
  \texttt{ollama\ pull\ nomic-embed-text} --- needed in Session 09
\item[$\square$]
  \textbf{The wifi test:} turn wifi off, run
  \texttt{ollama\ run\ qwen2.5-coder:3b\ "hello"}. It still answers
\item[$\square$]
  \textbf{aider} installed \textbf{outside} the course environment, via
  pipx --- \texttt{python3.12\ -m\ pip\ install\ -\/-user\ pipx},
  \texttt{python3.12\ -m\ pipx\ ensurepath}, new terminal,
  \texttt{pipx\ install\ aider-chat} (\texttt{py} for \texttt{python3}
  on Windows) --- never \texttt{pip\ install\ aider-chat} into
  \texttt{.venv}. \texttt{aider\ -\/-version} answers in a new terminal
\item[$\square$]
  \texttt{verify\_environment.py} passes \textbf{with the environment
  activated} --- the prompt shows \texttt{(.venv)}. Check 0 reports
  which interpreter is running; if it fails, every package check below
  it fails too and means nothing. Bring the output
\end{itemize}
...
\AttributeTok{    }\FunctionTok{model}\KeywordTok{:}\AttributeTok{ qwen2.5{-}coder:3b}
...
Dependency resolver errors while installing aider & It was installed
into \texttt{.venv}, where it competes with the course's packages &
Install it with pipx instead --- see the checklist above \\
Dependency conflicts installing aider, or a build error naming a
compiler & Your Python is 3.13 or newer; aider requires
\texttt{\textless{}3.13} & Give aider a 3.12 of its own:
\texttt{pipx\ install\ -\/-python\ python3.12\ aider-chat} \\
\texttt{pipx} or \texttt{aider} \emph{n'est pas reconnu} / \emph{not
recognized} & \texttt{pip\ install\ -\/-user} writes scripts to a folder
Windows does not have on PATH & \texttt{py\ -m\ pipx\ …} works
regardless; \texttt{py\ -m\ pipx\ ensurepath} plus a new terminal fixes
PATH itself \\
A newly installed extension or command does nothing & The window or
terminal predates the install & \emph{Developer: Reload Window}; for
PATH changes, quit VS Codium and reopen \\
\texttt{Python\ introuvable} / \texttt{Python\ was\ not\ found}
(Windows) & The Microsoft Store alias is shadowing Python & Start →
\textbf{Manage app execution aliases} → turn \textbf{off}
\texttt{python.exe} and \texttt{python3.exe}, then open a new terminal.
Do not install from the Store. \\
\texttt{python3:\ command\ not\ found} (Windows) & There is no
\texttt{python3} on Windows & Use \texttt{py}. If that is missing too,
re-run the python.org installer with \textbf{Add python.exe to PATH}
ticked \\
Continue answers nothing & Server not running & \texttt{ollama\ serve}
in a separate terminal \\
Model extremely slow & \texttt{ollama\ ps} shows 100\% CPU --- no
dedicated graphics card & Normal on most laptops with the 3b. A larger
model will be slower, not faster \\
aider ``forgets'' the file you gave it & The context window is too small
--- \texttt{ollama\ ps} shows it & Raise it in the \textbf{Ollama app's
own settings} (Settings → Context length), then quit Ollama from the
tray or menu bar and restart. The app overrides \texttt{setx} and
\texttt{launchctl}, so those appear to do nothing. Linux:
\texttt{systemctl\ edit\ ollama.service} \\
\texttt{address\ already\ in\ use}, or \emph{une seule utilisation de
chaque adresse de socket} & Ollama is already running; the installer
starts it on every platform & Do not run \texttt{ollama\ serve}. Set the
variable, then restart Ollama itself \\
\texttt{connection\ refused} from aider & Nothing listening on 11434 &
Start Ollama --- the app on macOS and Windows,
\texttt{sudo\ systemctl\ start\ ollama} on Linux \\
...
in your prompt \\
Windows: \emph{l'exécution de scripts est désactivée sur ce système} &
PowerShell's execution policy blocks \texttt{Activate.ps1} &
\texttt{Set-ExecutionPolicy\ -Scope\ CurrentUser\ -ExecutionPolicy\ RemoteSigned}
--- user-scoped, no admin rights, asked once \\
Windows: \emph{le répertoire n'est pas vide} when deleting
\texttt{.venv} & VS Codium holds the interpreter it has selected & Quit
VS Codium; delete from a plain PowerShell;
\texttt{cmd\ /c\ "rmdir\ /s\ /q\ .venv"} as a last resort \\
\texttt{deactivate} is not recognised & Activation defines it, so it
exists only inside an active environment & You are not in one. Skip it
--- the prompt shows \texttt{(.venv)} when you are \\
...
your group produces is committed on that branch and pushed to it, into
your group's own directory:
...
\texttt{groups/A2026/} ships \textbf{empty}. Your group creates its own
folder the first time it submits ---
\texttt{mkdir\ -p\ groups/A2026/group-XX/session-02} --- and committing
a file inside it is what puts your group in the repository. There is no
registration step, and git does not track an empty directory.

Your whole term is one directory, which is how you find in week ten what
you wrote in week two. The cohort folder above it is what lets next
year's students start clean rather than on top of you --- and it is why
the path is \texttt{A2026} and not just \texttt{groups/}.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python3.12} \AttributeTok{{-}m}\NormalTok{ venv .venv           }\CommentTok{\# Windows: py {-}3.12 {-}m venv .venv}
...
\FunctionTok{mkdir} \AttributeTok{{-}p}\NormalTok{ groups/A2026/group{-}07/session{-}02}
\BuiltInTok{echo} \StringTok{"\# Group 07 — session 02"} \OperatorTok{\textgreater{}}\NormalTok{ groups/A2026/group{-}07/session{-}02/NOTES.md}

\FunctionTok{git}\NormalTok{ add groups/A2026/group{-}07/session{-}02/}
...
\NormalTok{  groups/A2026/group{-}07/session{-}02/NOTES.md}

\FunctionTok{git}\NormalTok{ add groups/A2026/group{-}07/session{-}02/NOTES.md}
...
\ExtensionTok{aider} \AttributeTok{{-}{-}model}\NormalTok{ ollama\_chat/qwen2.5{-}coder:3b }\AttributeTok{{-}{-}no{-}auto{-}commits}
\end{Highlighting}
\end{Shaded}
...
Sessions 02--11 pair one \textbf{academic article} with the \textbf{data
behind it} --- a Harvard Dataverse replication package where the authors
deposited one, and otherwise the public database the paper itself is
built on, fetched by \texttt{qmib.load()}. Before class, read and
...
\section{Article--data map}\label{articledata-map}
...
05 · Regularisation & Blonigen \& Piger (2014), \emph{Determinants of
foreign direct investment} ·
\href{https://doi.org/10.1111/caje.12091}{article} ·
\href{https://www.nber.org/papers/w16704}{free preprint} & No package
published --- rebuilt by \texttt{scripts/build\_fdi\_determinants.py}
from OECD, CEPII, World Bank and Freedom House; load with
\texttt{qmib.load("fdi")} & The paper runs Bayesian model averaging over
56 candidate determinants of bilateral FDI and finds about sixteen
survive. Refit the same 56 for 2019 with lasso, ridge and elastic net,
and say which of its conclusions still hold. \\
...
07 · PCA and factor analysis & Gygli, Haelg, Potrafke \& Sturm (2019),
\emph{The KOF Globalisation Index --- revisited} ·
\href{https://doi.org/10.1007/s11558-019-09344-2}{article} & No deposit
--- the index itself is public; load with \texttt{qmib.load("kof")} &
The index asserts that globalisation has three dimensions --- economic,
social, political --- built from six sub-dimensions. Run PCA on the
sub-indices and ask how many the data actually supports. Defend the
number you retain, and interpret the loadings. \\
08 · KNN and bias--variance & Bluwstein, Buckmann, Joseph, Kapadia \&
Şimşek (2023), \emph{Credit growth, the yield curve and financial crisis
prediction} ·
\href{https://doi.org/10.1016/j.jinteco.2023.103773}{article} ·
\href{https://www.ecb.europa.eu/pub/pdf/scpwps/ecb.wp2614~6974517ac3.en.pdf}{free
working paper} & Jordà--Schularick--Taylor Macrohistory Database,
public; load with \texttt{qmib.load("jst")} & The paper reports that
flexible machine learning beats logistic regression at predicting
financial crises. Fit KNN against logistic regression on the same
18-country panel, choose \(k\) by cross-validation, and say whether
flexibility earned its keep on \textbf{your} feature set. \\
...
10 · Causal inference I & Atkin, Khandelwal \& Osman (2017),
\emph{Exporting and Firm Performance: Evidence from a Randomized
Experiment} · \href{https://doi.org/10.1093/qje/qjx002}{article} &
\href{https://doi.org/10.7910/DVN/QOGMVI}{10.7910/DVN/QOGMVI} & Egyptian
rug producers were randomly given export orders, so the counterfactual
is known. Reproduce the experimental estimate; then throw the
randomisation away, re-estimate by propensity-score matching on
observables, and report how far matching lands from the truth. \\
11 · Causal inference II & Cavallo, Gopinath, Neiman \& Tang (2021),
\emph{Tariff Passthrough at the Border and at the Store: Evidence from
US Trade Policy} · \href{https://doi.org/10.1257/aeri.20190536}{article}
& \href{https://doi.org/10.7910/DVN/JV7FCH}{10.7910/DVN/JV7FCH} & The
2018--19 US tariffs hit some goods and not others --- a
difference-in-differences design with a real policy as the treatment.
Reproduce one passthrough estimate, show the parallel-trends evidence,
and compare conventional with design-aware uncertainty. \\
\end{longtable}
...
Session 05 is the exception: its authors published no replication
package, so the course rebuilt their design from the sources the paper
names. It needs no download --- \texttt{qmib.load("fdi")} reads a file
committed to the repository.
\texttt{scripts/build\_fdi\_determinants.py} documents every source and
every substitution.
...
{Approach} to {Parallel} {Trends}.''} \emph{{Review} of {Economic}
{Studies}} 90 (5): 2555--91.
