...
Firms discovered the shape of their own supply chains by watching them break. The research
literature had been arguing for years that concentration and fragility in global production were
badly measured and badly understood; what it lacked was an event that made the point unarguable
[@baldwin2022].
...
not trade; firms do. This is not a rhetorical point: heterogeneous-firm trade theory reorganised
the field around it, showing that aggregate trade responds mainly through which firms enter and
leave export markets rather than through all firms adjusting together [@melitz2003; @warin-firms].
Aggregate statistics on bilateral flows are the sum of those decisions, taken under constraints —
freight, insurance, policy wedges, the cost of switching a supplier — that the aggregate cannot
show. The gravity equation remains the workhorse for describing those flows [@anderson2003], and
describing them is not the same as judging whether they are the ones that should be happening
[@warin-gravity]. Work at the level where the decision is actually made requires data at that
level, and methods equal to it.
...
The methods have moved with it. Econometrics has absorbed a toolkit built for prediction rather
than for inference, and the two demand different care [@varian2014]; the data themselves are
increasingly administrative, transactional and unstructured rather than surveyed [@einav2014].
Quantities nobody could measure at all are now constructed — economic policy uncertainty read off
newspaper archives has entered mainstream macroeconomics [@baker2016], and the productive
capabilities of a whole economy can be inferred from the composition of what it exports
[@hidalgo2009]. None of this removes the need for the discipline this book teaches. It multiplies
the number of places to go wrong.
...
course resolves it in favour of the mess [@warin-statcan]. Every technique here is applied to a
panel carrying the missing values, the influential observations and the awkward panel structure of
the real European sources — as teaching fixtures generated from a known structure, so that the
methods behave as they would on the real series while no number here can be mistaken for a fact
about Europe. Chapter 1 says what that means and what it costs.
:::
