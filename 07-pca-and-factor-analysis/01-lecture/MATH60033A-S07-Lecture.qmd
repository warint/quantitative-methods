...
3. The Netflix's Recommender System through a PCA
4. Factor analysis
5. FAMD: the wine data
6. Factor Analysis for Questionnaires
...
## Introduction {#imported-s07-005 .center .scrollable}

## Where this session sits {#imported-s07-006 .scrollable}
...
## The machine-learning workflow {#imported-s07-009 .scrollable}
...
## Regression or classification? {#imported-s07-010 .scrollable}
...
-   **Regression** predicts a number: a price, a volume, a share.

-   **Classification** predicts a category: default or not, beer or wine. A classifier draws a boundary between the classes.

![](economist-assets/bayes-classifier.png){style="display: block; margin: auto;" width="60%"}
:::
...
## Principal Component Analysis {#imported-s07-013 .center .scrollable}

## Why PCA? {#imported-s07-015 .scrollable}
...
## What PCA does {#imported-s07-016 .scrollable}
...
-   Categorical data must be encoded as numeric data (e.g. \"one-hot encoding\" = creating dummies from categories, `pd.get_dummies`)

-   Numeric data must be scaled (otherwise your PCA will be misleading)
:::

## The Netflix\'s Recommender System through a PCA {#imported-s07-020 .center .scrollable}

## Step 1: Collecting the data {#imported-s07-021 .scrollable}
...
## Step 2: Computing the principal components {#imported-s07-023 .scrollable}
...
## Step 3: Which variables contribute to the principal components? {#imported-s07-025 .scrollable}
...
## Step 3 (cont.): The variables, drawn {#imported-s07-027 .scrollable}
...
# Variable plot: arrows on the unit circle
...
## Step 4: Where each film sits — the scores {#imported-s07-030 .scrollable}
...
## Step 4 (cont.): The films, drawn {#imported-s07-031 .scrollable}
...
## Factor analysis {#imported-s07-033 .center .scrollable}

## What factor analysis assumes {#imported-s07-034 .scrollable}
...
## The factor model {#imported-s07-035 .scrollable}
...
## FAMD: the wine data {#imported-s07-037 .center .scrollable}

## Fitting FAMD {#imported-s07-038 .scrollable}
...
## Variable coordinates {#imported-s07-040 .scrollable}
...
## The wines, drawn {#imported-s07-043 .scrollable}

```{python}
#| echo: true

# Individuals, coloured by the verdict
...
## Factor Analysis for Questionnaires {#imported-s07-044 .center .scrollable}

## The cars questionnaire {#imported-s07-045 .scrollable}
...
## Three factors or four? {#imported-s07-047 .scrollable}
...
## Which items load where {#imported-s07-049 .scrollable}

```{python}
#| echo: true

# The same table read as a list: each item under the factor it loads on.
...
## Naming the factors {#imported-s07-050 .scrollable}

<!-- The names follow the four-factor varimax solution the slide before prints.
     If the data or the rotation change, re-read that list before these. -->

| Factor | Items that load on it | Name |
|--------|-----------------------|------|
| F1 | color, exterior_looks | Aesthetics |
| F2 | space_comfort, fuel_type, after_sales_service, technology, safety | Functional benefits |
| F3 | resale_value, maintenance, price, fuel_efficiency | Economic value |
| F4 | testimonials, test_drive, product_reviews | Credibility |

-   The **sign** of a factor is arbitrary: F3 and F4 load negatively on all their items, and flipping the whole column changes nothing. What matters is which items move together.

-   Within F1, `color` and `exterior_looks` have **opposite** signs, so F1 is a contrast between the two rather than a sum.

-   `fuel_efficiency`, `maintenance` and `product_reviews` load on two factors. The name is a claim you defend, not something the software returns.
